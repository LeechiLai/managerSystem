<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="DesignRatioList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="项目收款金额" align="center" prop="receivedAmount" />
      <el-table-column label="专业分配金额" align="center">
        <template #default="scope">
          {{ calculateMajorAmount(scope.row) }}
        </template>
      </el-table-column>
      <el-table-column label="个人分配金额" align="center">
        <template #default="scope">
          {{ calculatePersonAmount(scope.row) }}
        </template>
      </el-table-column>
...
// 添加数据验证
const validateRatios = (rule, value, callback) => {
  // 验证同一专业下所有人员比例之和是否为1
  const sameMajorRatios = DesignRatioList.value
      .filter(item => item.prjId === form.value.prjId && item.profession === form.value.profession)
      .reduce((sum, item) => sum + (item.internalPct || 0), 0);

  if (sameMajorRatios > 1) {
    callback(new Error('同一专业下所有人员比例之和不能超过1'));
  } else {
    callback();
  }
};
...
const data = reactive({
  ...
  rules: {
    ...
    internalPct: [{ required: true, message: "设计组内部比例不能为空", trigger: "blur" }],
    projPct: [{ required: true, message: "设计组占总项目比例不能为空", trigger: "blur" }],
    personRatio: [
      { required: true, message: "个人比例不能为空", trigger: "blur" },
      { validator: validateRatios, trigger: "blur" }
    ]
  }
});
